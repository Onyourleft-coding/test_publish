...
/****************************************AES************************************/
const CRYPTOJSKEY = "81eba4f68353e6d5";
...
export function encrypt(word) {
  let key = enc.Utf8.parse(CRYPTOJSKEY);
...
//向量,对应TripleDES.java 	private final static String iv = "12345678";
const des3iv = "12345678";

// 解密，加密的key
const DES3_KEY = `abcdefghij1234abcdefghij`;
...
function genkey(key, start, end) {
  return { key: padd(key.slice(start, end)), vector: 1 };
}
...
export const DES3 = {
  ...
  encrypt(str) {
    ...
    try {
      var genKey = genkey(DES3_KEY, 0, 24);
      return btoa(des(genKey.key, str, 1, 1, des3iv, 1));
    } catch (error) {
      return "";
    }
  },
  ...
  decrypt(str) {
    ...
    try {
      var genKey = genkey(DES3_KEY, 0, 24);
      return des(genKey.key, atob(str), 0, 1, des3iv, 1);
    } catch (error) {
      return "";
    }
  },
